...
const STAGE_COLORS: Record<string, string> = {
  data_loading:      'bg-slate-500/15   border-slate-500/30   text-slate-400',
  ...
  return (
    <div className="card p-5 animate-fade-in">
      ...
      <div className="relative">
        {/* Vertical connector */}
        <div className="absolute left-4 top-4 bottom-4 w-px bg-surface-400" />

        <div className="space-y-2 ml-10">
          {steps.map((step, i) => {
            const dotCls  = STAGE_DOT[step.stage]  ?? 'bg-slate-400'
            const tagCls  = STAGE_COLORS[step.stage] ?? 'bg-slate-500/15 border-slate-500/30 text-slate-400'
            ...
            return (
              <div key={i} className="relative">
                {/* Node dot */}
                <div className={`absolute -left-[2.25rem] top-3 w-2.5 h-2.5 rounded-full border-2 border-surface-900 ${dotCls}`} />
